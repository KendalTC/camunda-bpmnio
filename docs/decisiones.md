...
- [x] Subproceso colapsado "Validar pago" + Boundary Timer — ⚠️ descartado en la Fase 6 (se volvió a un flujo plano por legibilidad); el timer se mantiene sobre el tramo de validación de pago
- [x] Gateway "¿Pedido aprobado?" conectando la decisión del revisor (rechazar / solicitar más info / aprobar) al resto del proceso, y Consigna 2 (doble Boundary Timer + escalamiento por atraso en la entrega) — ver [`03-modelado/reglas-dmn.md`](../03-modelado/reglas-dmn.md#fase-6--versión-final-gateway-del-revisor--consigna-2-sla-de-entrega)
- [ ] **2 bugs detectados por revisión de XML, pendientes de corregir:** (1) el gateway "¿Pedido aprobado?" no tiene flujo por defecto, por lo que un pedido sin revisión manual (`decisionRevision` indefinido) fallaría en ese punto; (2) "Entrega escalada a logística" es un Intermediate Throw Event sin salida, no un End Event — debería convertirse en End Event de tipo Escalation. Detalle en la Fase 6.
...
- [ ] Redactar consignas formales para compañeros y preparar `Workshop_V1.bpmn` como plantilla incompleta (ver [`05-workshop/tarea-companeros.md`](../05-workshop/tarea-companeros.md))
- [ ] Guion cronometrado del workshop (80 min) — el modelo ya es extenso, decidir qué tramos se reconstruyen en vivo vs. cuáles se muestran ya hechos
